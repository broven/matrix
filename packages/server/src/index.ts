...
// Explicitly exclude X-Matrix-Internal from allowHeaders to prevent malicious websites
// from bypassing loopback security checks via CORS preflight requests.
app.use(
  "/*",
  cors({
...
export function isLoopbackRequest(c: any): boolean {
  const isLoopbackIp =
    ...
    c.env?.incoming?.socket?.remoteAddress === "::1" ||
    c.env?.incoming?.socket?.remoteAddress === "::ffff:127.0.0.1";

  if (!isLoopbackIp) return false;

  // For added security, we require a special header that cannot be set by a browser
  // during a cross-origin request (due to our CORS configuration).
...
log.info(
  { host: config.host, port: config.port, token: maskToken(serverToken) },
